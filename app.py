...
def face_recognition():  # generate frame by frame from camera
    ...
    # Create an empty list to store recognized participant IDs
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
        ...
        for (x, y, w, h) in features:
            ...
            s = s or ''
            s = '' + ''.join(list(s))
            if s not in recognized_names:
                recognized_names.append(s)
            ...
            coords = [x, y, w, h]
            check_availability(recognized_names)
    ...
    def recognize(img, clf, faceCascade):
        ...
        # Recognize the faces in the frame and draw bounding boxes
        coords = draw_boundary(img, faceCascade, 1.1, 10, (255, 255, 0), "Face", clf)
        return img

    # Initialize the necessary OpenCV objects
...
def stop_program():
    "This functions helps to check the availability"
    sys.exit()
